...
import { getGroqChatCompletion } from "./api/api";
...
export default function Chat() {
  ...
  const sendMessage = async () => {
    if (userInput.trim()) {
      ...
      if (botMessage) {
        addMessage({ message: botMessage, type: "bot" });
      }
    }
  };
                ...
                >
                  ...
                  <div
                    className={`max-w-[65%] px-4 py-2 rounded-lg text-white text-sm ${
                      msg.type === "bot"
                        ? "bg-gray-700/50 backdrop-blur-sm shadow-lg border border-gray-600/50"
                        : "bg-purple-500/50 backdrop-blur-sm shadow-lg"
                    }`}
                  >
                    {msg.type === "bot" ? (
