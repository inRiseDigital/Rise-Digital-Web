...
const HeroContent = () => {
  ...
  const variants: Variants = {
    float: {
      y: [-20, 20], // Ensuring it's correctly typed
      transition: {
        duration: 2,
        repeat: Infinity,
        repeatType: "reverse", // Must be "reverse", "loop", or "mirror"
        ease: "easeInOut"
      }
    }
  };
  ...
  return (
    ...
    >
      ...
      <motion.div
        variants={variants}
        animate="float"
        className="w-full h-full flex justify-center items-center order-first lg:order-last mb-8 lg:mb-0"
      >
        <Image
          src="/home/hero.png"
          alt="work icons"
      ...
      <div className="h-full w-full flex flex-col gap-5 justify-center m-auto text-start order-last lg:order-first -mt-4">
  <motion.div
  ...
  >
    ...
    <p className={`text-[24px] md:text-[28px] lg:text-[32px] leading-[30px] md:leading-[36px] lg:leading-[44px] text-[#7d8590] font-heading3 ${subtitle.className}`}>
      {isClient ? (
        <Typewriter
          options={{
            strings: [
    ...
    <div className="mt-2"> {/* Reduced from mt-8 to mt-4 */}
      <a
